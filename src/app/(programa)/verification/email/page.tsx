...
function EmailVerifyContent() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setNewSend(true), 10000);
  ...
  return (
    <div className="py-3 text-white px-8 min-h-screen bg-gradient-to-br from-[#2825eb] via-[#1915eb] to-[#5403d6]">
      <header className="flex items-center justify-between h-7 mb-10">
        <Logo />
        <Button onClick={() => router.push("/home")} className="cursor-pointer text-white" variant="link">
          Sair
        </Button>
      </header>
      <motion.section
      ...
      >
        <div className="max-w-lg md:mx-auto md:border md:border-white/40 md:rounded-md md:shadow-2xl md:p-5">
          ...
          >
            <InputOTP maxLength={6} value={codOTP} onChange={(e) => setCodOTP(e)}>
              <InputOTPGroup>
          ...
          {error && <p className="text-xs text-center p-1 sm:text-sm text-red-500">{error}</p>}
          <div>
            <Button onClick={handleVerify} className="bg-pink-500 cursor-pointer w-full mb-4">
              Confirmar
            </Button>
            {newSend && (
...
export default function EmailVerify() {
  return (
    <Suspense fallback={<div className="text-center mt-10 text-white">Carregando...</div>}>
      <EmailVerifyContent />
    </Suspense>
  );
}